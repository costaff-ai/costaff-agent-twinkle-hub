"""The shared manager-core tools as native function tools.
...
the costaff-mcp container; nothing duplicated here. Replaces the old
`costaff` entry in TWINKLE_HUB_AGENT_MCP_URLS (one fewer MCP session).
"""
import os
...
def send_message_now(
    ...
    [PROGRESS_CONTEXT] block. Prefix body with [Twinkle], plain text, no
    emoji. channel/recipient/user_id/session_id come from [PROGRESS_CONTEXT].
    """
    return call_shim(
...
def add_task_comment(
    ...
    author: 'user' or 'twinkle_hub_agent'.
